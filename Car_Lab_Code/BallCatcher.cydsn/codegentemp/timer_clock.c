...
{
    if (timer_clock_CLKEN & timer_clock_CLKEN_MASK)
    {
...
*
*******************************************************************************/
void timer_clock_SetModeRegister(uint8 clkMode) 
{
    timer_clock_MOD_SRC |= clkMode & timer_clock_MODE_MASK;
}
...
*
*******************************************************************************/
void timer_clock_ClearModeRegister(uint8 clkMode) 
{
    timer_clock_MOD_SRC &= ~clkMode | ~timer_clock_MODE_MASK;
}
...
{
    return timer_clock_MOD_SRC & timer_clock_MODE_MASK;
}
...
void timer_clock_SetSourceRegister(uint8 clkSource) 
{
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        timer_clock_MOD_SRC |= CYCLK_SSS;
        timer_clock_MOD_SRC =
            (timer_clock_MOD_SRC & ~timer_clock_SRC_SEL_MSK) | clkSource;
    }
