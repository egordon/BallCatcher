...
/*******************************************************************************
* Function Name: pan_reset_clk_Start
********************************************************************************
* Summary:
...
void pan_reset_clk_Start(void) 
{
    ...
    pan_reset_clk_CLKEN |= pan_reset_clk_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: pan_reset_clk_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: pan_reset_clk_StopBlock
********************************************************************************
* Summary:
...
{
    if (pan_reset_clk_CLKEN & pan_reset_clk_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = pan_reset_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = pan_reset_clk_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(pan_reset_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(pan_reset_clk_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: pan_reset_clk_StandbyPower
********************************************************************************
* Summary:
...
void pan_reset_clk_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        pan_reset_clk_CLKSTBY &= ~pan_reset_clk_CLKSTBY_MASK;
    }
    else
    {
        pan_reset_clk_CLKSTBY |= pan_reset_clk_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: pan_reset_clk_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void pan_reset_clk_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = pan_reset_clk_GetSourceRegister();
    uint16 oldDivider = pan_reset_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = pan_reset_clk_CLKEN & pan_reset_clk_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(pan_reset_clk_DIV_PTR, clkDivider);
                pan_reset_clk_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: pan_reset_clk_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(pan_reset_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: pan_reset_clk_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void pan_reset_clk_SetModeRegister(uint8 clkMode) 
{
    pan_reset_clk_MOD_SRC |= clkMode & pan_reset_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: pan_reset_clk_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void pan_reset_clk_ClearModeRegister(uint8 clkMode) 
{
    pan_reset_clk_MOD_SRC &= ~clkMode | ~pan_reset_clk_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: pan_reset_clk_GetModeRegister
********************************************************************************
* Summary:
...
{
    return pan_reset_clk_MOD_SRC & pan_reset_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: pan_reset_clk_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = pan_reset_clk_GetDividerRegister();
    uint8 oldSrc = pan_reset_clk_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        pan_reset_clk_MOD_SRC |= CYCLK_SSS;
        pan_reset_clk_MOD_SRC =
            (pan_reset_clk_MOD_SRC & ~pan_reset_clk_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: pan_reset_clk_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return pan_reset_clk_MOD_SRC & pan_reset_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: pan_reset_clk_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    pan_reset_clk_PHASE = clkPhase & pan_reset_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: pan_reset_clk_GetPhase
********************************************************************************
* Summary:
...
{
    return pan_reset_clk_PHASE & pan_reset_clk_PHASE_MASK;
}
