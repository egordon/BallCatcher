...
void timer_clock_2_Start(void) 
{
    ...
    timer_clock_2_CLKEN |= timer_clock_2_CLKEN_MASK;
}
...
/*******************************************************************************
* Function Name: timer_clock_2_StopBlock
********************************************************************************
* Summary:
...
{
    if (timer_clock_2_CLKEN & timer_clock_2_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = timer_clock_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = timer_clock_2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(timer_clock_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(timer_clock_2_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: timer_clock_2_StandbyPower
********************************************************************************
* Summary:
...
void timer_clock_2_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        timer_clock_2_CLKSTBY &= ~timer_clock_2_CLKSTBY_MASK;
    }
    else
    {
        timer_clock_2_CLKSTBY |= timer_clock_2_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: timer_clock_2_SetDividerRegister
********************************************************************************
* Summary:
...
void timer_clock_2_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    ...
    uint8 currSrc = timer_clock_2_GetSourceRegister();
    uint16 oldDivider = timer_clock_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = timer_clock_2_CLKEN & timer_clock_2_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(timer_clock_2_DIV_PTR, clkDivider);
                timer_clock_2_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
{
    return CY_GET_REG16(timer_clock_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: timer_clock_2_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void timer_clock_2_SetModeRegister(uint8 clkMode) 
{
    timer_clock_2_MOD_SRC |= clkMode & timer_clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_2_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void timer_clock_2_ClearModeRegister(uint8 clkMode) 
{
    timer_clock_2_MOD_SRC &= ~clkMode | ~timer_clock_2_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: timer_clock_2_GetModeRegister
********************************************************************************
* Summary:
...
{
    return timer_clock_2_MOD_SRC & timer_clock_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_2_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = timer_clock_2_GetDividerRegister();
    uint8 oldSrc = timer_clock_2_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        timer_clock_2_MOD_SRC |= CYCLK_SSS;
        timer_clock_2_MOD_SRC =
            (timer_clock_2_MOD_SRC & ~timer_clock_2_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: timer_clock_2_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return timer_clock_2_MOD_SRC & timer_clock_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: timer_clock_2_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    timer_clock_2_PHASE = clkPhase & timer_clock_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: timer_clock_2_GetPhase
********************************************************************************
* Summary:
...
{
    return timer_clock_2_PHASE & timer_clock_2_PHASE_MASK;
}
