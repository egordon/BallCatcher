...
{
    if (PWM_clk_CLKEN & PWM_clk_CLKEN_MASK)
    {
...
*
*******************************************************************************/
void PWM_clk_SetModeRegister(uint8 clkMode) 
{
    PWM_clk_MOD_SRC |= clkMode & PWM_clk_MODE_MASK;
}
...
*
*******************************************************************************/
void PWM_clk_ClearModeRegister(uint8 clkMode) 
{
    PWM_clk_MOD_SRC &= ~clkMode | ~PWM_clk_MODE_MASK;
}
...
{
    return PWM_clk_MOD_SRC & PWM_clk_MODE_MASK;
}
...
void PWM_clk_SetSourceRegister(uint8 clkSource) 
{
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        PWM_clk_MOD_SRC |= CYCLK_SSS;
        PWM_clk_MOD_SRC =
            (PWM_clk_MOD_SRC & ~PWM_clk_SRC_SEL_MSK) | clkSource;
    }
