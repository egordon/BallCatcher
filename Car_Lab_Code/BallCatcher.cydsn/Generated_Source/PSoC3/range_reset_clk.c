...
/*******************************************************************************
* Function Name: range_reset_clk_Start
********************************************************************************
* Summary:
...
void range_reset_clk_Start(void) 
{
    ...
    range_reset_clk_CLKEN |= range_reset_clk_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: range_reset_clk_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: range_reset_clk_StopBlock
********************************************************************************
* Summary:
...
{
    if (range_reset_clk_CLKEN & range_reset_clk_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = range_reset_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = range_reset_clk_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(range_reset_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(range_reset_clk_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: range_reset_clk_StandbyPower
********************************************************************************
* Summary:
...
void range_reset_clk_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        range_reset_clk_CLKSTBY &= ~range_reset_clk_CLKSTBY_MASK;
    }
    else
    {
        range_reset_clk_CLKSTBY |= range_reset_clk_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: range_reset_clk_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void range_reset_clk_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = range_reset_clk_GetSourceRegister();
    uint16 oldDivider = range_reset_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = range_reset_clk_CLKEN & range_reset_clk_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(range_reset_clk_DIV_PTR, clkDivider);
                range_reset_clk_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: range_reset_clk_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(range_reset_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: range_reset_clk_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void range_reset_clk_SetModeRegister(uint8 clkMode) 
{
    range_reset_clk_MOD_SRC |= clkMode & range_reset_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: range_reset_clk_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void range_reset_clk_ClearModeRegister(uint8 clkMode) 
{
    range_reset_clk_MOD_SRC &= ~clkMode | ~range_reset_clk_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: range_reset_clk_GetModeRegister
********************************************************************************
* Summary:
...
{
    return range_reset_clk_MOD_SRC & range_reset_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: range_reset_clk_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = range_reset_clk_GetDividerRegister();
    uint8 oldSrc = range_reset_clk_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        range_reset_clk_MOD_SRC |= CYCLK_SSS;
        range_reset_clk_MOD_SRC =
            (range_reset_clk_MOD_SRC & ~range_reset_clk_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: range_reset_clk_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return range_reset_clk_MOD_SRC & range_reset_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: range_reset_clk_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    range_reset_clk_PHASE = clkPhase & range_reset_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: range_reset_clk_GetPhase
********************************************************************************
* Summary:
...
{
    return range_reset_clk_PHASE & range_reset_clk_PHASE_MASK;
}
