...
/*******************************************************************************
* Function Name: tilt_reset_clk_Start
********************************************************************************
* Summary:
...
void tilt_reset_clk_Start(void) 
{
    ...
    tilt_reset_clk_CLKEN |= tilt_reset_clk_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: tilt_reset_clk_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: tilt_reset_clk_StopBlock
********************************************************************************
* Summary:
...
{
    if (tilt_reset_clk_CLKEN & tilt_reset_clk_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = tilt_reset_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = tilt_reset_clk_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(tilt_reset_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(tilt_reset_clk_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: tilt_reset_clk_StandbyPower
********************************************************************************
* Summary:
...
void tilt_reset_clk_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        tilt_reset_clk_CLKSTBY &= ~tilt_reset_clk_CLKSTBY_MASK;
    }
    else
    {
        tilt_reset_clk_CLKSTBY |= tilt_reset_clk_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: tilt_reset_clk_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void tilt_reset_clk_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = tilt_reset_clk_GetSourceRegister();
    uint16 oldDivider = tilt_reset_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = tilt_reset_clk_CLKEN & tilt_reset_clk_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(tilt_reset_clk_DIV_PTR, clkDivider);
                tilt_reset_clk_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: tilt_reset_clk_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(tilt_reset_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: tilt_reset_clk_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void tilt_reset_clk_SetModeRegister(uint8 clkMode) 
{
    tilt_reset_clk_MOD_SRC |= clkMode & tilt_reset_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: tilt_reset_clk_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void tilt_reset_clk_ClearModeRegister(uint8 clkMode) 
{
    tilt_reset_clk_MOD_SRC &= ~clkMode | ~tilt_reset_clk_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: tilt_reset_clk_GetModeRegister
********************************************************************************
* Summary:
...
{
    return tilt_reset_clk_MOD_SRC & tilt_reset_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: tilt_reset_clk_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = tilt_reset_clk_GetDividerRegister();
    uint8 oldSrc = tilt_reset_clk_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        tilt_reset_clk_MOD_SRC |= CYCLK_SSS;
        tilt_reset_clk_MOD_SRC =
            (tilt_reset_clk_MOD_SRC & ~tilt_reset_clk_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: tilt_reset_clk_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return tilt_reset_clk_MOD_SRC & tilt_reset_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: tilt_reset_clk_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    tilt_reset_clk_PHASE = clkPhase & tilt_reset_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: tilt_reset_clk_GetPhase
********************************************************************************
* Summary:
...
{
    return tilt_reset_clk_PHASE & tilt_reset_clk_PHASE_MASK;
}
